...
class Track:
    ...
    def __init__(
        ...
        # self.tr_rights = None
        # self.tr_lefts = None
        # self.tr_rights_jax = jnp.array(tr_rights) if tr_rights else None
        # self.tr_lefts_jax = jnp.array(tr_lefts) if tr_lefts else None
        self.s_guess = None
        self.frenet_search_range = 10 # meters
        self.s_frame_max = centerline.s_frame_max
    ...
    def curvature(self, s):
        ...
        return self.centerline.spline.calc_curvature(s)
    
    @partial(jax.jit, static_argnums=(0))
    def curvature_jax(self, s):
        s = s % self.s_frame_max
        return self.centerline.spline.calc_curvature_jax(s)
    
    def get_s_search_range(self, s_guess, s_ind_range):
        s_ind = self.centerline.spline.find_segment_for_s(s_guess)
        s_inds = (np.arange(s_ind, s_ind + s_ind_range) % self.centerline.length).astype(int)
        return s_inds
    ...
    def frenet_to_cartesian_jax(self, s, ey, ephi):
        ...
        """
        s = s % self.s_frame_max
        x, y = self.centerline.spline.calc_position_jax(s)
        psi = self.centerline.spline.calc_yaw_jax(s)
    ...
    def cartesian_to_frenet_jax_jit(self, pose, s_inds):
        ...
        s, ey = self.centerline.spline.calc_arclength_jax(x, y, s_inds)
        # Wrap around
        s = s % self.s_frame_max
        
        # Use the normal to calculate the signed lateral deviation
        # normal = self.centerline._calc_normal(s)
        yaw = self.centerline.spline.calc_yaw_jax(s)
        normal = jnp.asarray([-jnp.sin(yaw), jnp.cos(yaw)])
        x_eval, y_eval = self.centerline.spline.calc_position_jax(s)
        dx = x - x_eval
        dy = y - y_eval
