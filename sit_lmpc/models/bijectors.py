...
class TransformedConditional(Transformed):
    ...
    def sample(self, x, context: Optional[Array] = None) -> Array:
        y, _ = self.bijector.forward_and_log_det(x, context)
        return y

    def sample_base(self, seed: PRNGKey, sample_shape: List[int], context: Optional[Array] = None) -> Array:
        x = self.distribution.sample(seed=seed, sample_shape=sample_shape)
        y, _ = self.bijector.forward_and_log_det(x, context)
        return y
    
    def sample_dist(self, sample_dist, seed: PRNGKey, sample_shape: List[int], context: Optional[Array] = None) -> Array:
        x = sample_dist.sample(seed=seed, sample_shape=sample_shape)
        y, _ = self.bijector.forward_and_log_det(x, context)
        return y
